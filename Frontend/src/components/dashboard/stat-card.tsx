  ...
  return (
    <div className={cn(
      ...
      <div className="flex items-center justify-between mb-4">
        <span className="text-[12px] sm:text-[14px] font-bold text-slate-500 tracking-wide">{label}</span>
        <div className={cn('flex h-10 w-10 items-center justify-center rounded-xl shadow-sm', styles[variant].iconBg)}>
          {icon}
        </div>
      </div>
      <div>
        <p className="text-[24px] sm:text-[32px] font-black tracking-tight text-[#1C3F35] leading-none mb-1.5">{value}</p>
